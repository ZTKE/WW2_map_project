...
namespace HexasphereGrid {
    ...
    public partial class Hexasphere : MonoBehaviour {
                    ...
                    if (Application.isMobilePlatform) {
                        if (input.TouchCount > 0) {
                            ...
                            if (touch.phase == TouchPhase.Began) {
                                canInteract = !input.IsPointerOverUI(input.GetFingerIdFromTouch(0));
                            }
                        }
                    } else {
                        canInteract = !input.IsPointerOverUI();
                    }
                } else {
                    canInteract = true;
                }
        ...
        void CheckMouseIsOver() {
            ...
            if (_cameraMain != null) {
                Ray ray = _cameraMain.ScreenPointToRay(mousePos);
                int hitCount = Physics.RaycastNonAlloc(ray, pointer_hits);
                if (hitCount > 0) {
                    for (int k = 0; k < hitCount; k++) {
                        if (pointer_hits[k].collider.gameObject == gameObject) {
                            mouseIsOver = true;
                            return;
                        }
                    }
