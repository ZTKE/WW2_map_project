...
namespace HoneyFramework {
    ...
    public class GameManager : MonoBehaviour {
        ...
        // public int currentSeed = 0; // 已弃用
        public UnityEngine.Random.State currentSeed;
        ...
        void Start() {
            if (!initializedSeed) {
                ...
                if (settingsSeed != 0) {
                    // UnityEngine.Random.seed = settingsSeed; // 已弃用
                    UnityEngine.Random.InitState(settingsSeed);
                } else if (World.GetInstance().seed != 0) {
                    // UnityEngine.Random.seed = World.GetInstance().seed; // 已弃用
                    UnityEngine.Random.InitState(World.GetInstance().seed);
                }

                // currentSeed = UnityEngine.Random.seed; // 已弃用
