    ...
    public class TextureUtils {
        ...
        static public TextureImporterSettings ChangeTextureSettings(string path,
            ...
            if (File.Exists(path)) {
                ...
                // setting.textureFormat = format; // 已弃用
                setting.alphaIsTransparency = alphaIsTransparency;
                setting.filterMode = FilterMode.Bilinear;
                // setting.maxTextureSize = maxTextureSize; // 已弃用
                // setting.ApplyTextureType(type, apply); // 已弃用
                setting.ApplyTextureType(type);
                textureImporter.SetTextureSettings(setting);

                // 新增平台设置
                TextureImporterPlatformSettings p = textureImporter.GetDefaultPlatformTextureSettings();
                p.format = format;
                p.maxTextureSize = maxTextureSize;
